	...
	public void action() {
		...
		case 1:
			...
			for (DFAgentDescription bakery : bakeries) {
				Util.sendMessage(myAgent, bakery.getName(), ACLMessage.CFP,
						aggregatedOrders.firstKey() + " " + customer.getGuid(), "place-order");
			}
		...
		case 3:
			if (aggregatedBakeryResponses.size() == 0) {
				...
				AID bestBakery = Util.searchInYellowPage(myAgent, "BakeryService",
						aggregatedBakeryResponses.firstKey())[0].getName();

				Util.sendMessage(myAgent, bestBakery, ACLMessage.ACCEPT_PROPOSAL,
						aggregatedOrders.firstKey() + " " + customer.getGuid(), "place-order");
		...
		case 4:
			...
			if (confirmationMessage != null) {
				...
				if (confirmationMessage.getPerformative() == ACLMessage.CONFIRM) {
					...
					logger.info("Order {} successfully accepted by {}", parsedConfirmationMessage[1],
							parsedConfirmationMessage[0]);
