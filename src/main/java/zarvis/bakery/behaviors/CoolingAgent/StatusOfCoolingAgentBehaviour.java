...
public class StatusOfCoolingAgentBehaviour extends CyclicBehaviour {
	...
	public void action() {
		...
		if (message != null) {
			if (message.getPerformative() == CustomMessage.REQUEST_STATUS && message.getConversationId().equals("coolingMachine-availability")) {
				ACLMessage reply = message.createReply();
				reply.setPerformative(CustomMessage.RESPONSE_STATUS);
				reply.setContent(status.getStatus() ? "Available" : "Unavailable");
				reply.setConversationId("coolingMachine-status");
				myAgent.send(reply);
			}
